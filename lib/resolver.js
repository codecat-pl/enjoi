...
class SchemaResolver {
    ...
    resolveType(schema) {
        ...
        const joitype = (type, format, schema) => {
            let joischema;

            if (this.refineType) {
                type = this.refineType(type, format, schema);
            }
        ...
        if (Util.isArray(schema.type)) {
            ...
            for (let i = 0; i < schema.type.length; i++) {
                schemas.push(joitype(schema.type[i], schema.format, schema));
            }

            joischema = this.joi.alternatives(schemas);
        }
        else {
            joischema = joitype(schema.type, schema.format, schema);
        }
